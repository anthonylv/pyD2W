...
class Database:
    ...
    def get_drupal_duplicate_term_names(self):
        ...
        term_names = None
        try:
            print "Getting duplicate term names........"
            term_names = self.query(
                "SELECT term_data.tid, term_data.name, COUNT(*) c "
                "FROM term_data GROUP BY term_data.name, term_data.tid HAVING c > 1"
            )
        except mdb.ProgrammingError as ex:
            self._logger.error(
    ...
    def get_drupal_duplicate_terms(self):
        ...
        try:
            duplicate_terms = self.query(
                ...
                "FROM term_data "
                "INNER JOIN ( SELECT term_data.name FROM term_data "
                "GROUP BY term_data.name, term_data.tid HAVING COUNT(name) >1 ) temp "
    ...
    def get_duplicate_aliases(self):
        ...
        try:
            aliases = self.query(
                "SELECT url_alias.pid, url_alias.src, COUNT(*) c "
                "FROM url_alias GROUP BY url_alias.src, url_alias.pid HAVING c > 1;"
            )
        except mdb.ProgrammingError as ex:
            self._logger.error(
